...
- **Signals** (computed in `src/backtest.py`):
  ...
  - **Risk‑adjusted momentum score**: $\text{mom}_{score}(i, T) = \frac{\text{raw}_{mom}(i, T)}{\text{vol}_{11m}(i, T)}$
...
- **Position sizing**:
  ...
  - Longs: weights $\propto 1 / \text{vol}_{1m}$, normalized to sum to +1.0.

  - Shorts: weights $\propto 1 / \text{vol}_{1m}$, normalized to sum to −1.0.

  - Net exposure approximately 0, gross exposure $\approx 2$.
